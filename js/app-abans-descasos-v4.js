...
(() => {
  ...
  // afegit 2
    ...
    descansos = await fetchJsonWithFallback(["../data/descansos.json","data/descansos.json"]);
...
function colorForSubjectItem(s){
  ...
  // ---------- Render ----------
  function hasSessionsBeforeAndAfter(daySessions, breakItem) {
  ...
  function getActiveBreaksForDay(daySessions) {
    if (!daySessions || !daySessions.length) return [];
    return descansos.filter(b => hasSessionsBeforeAndAfter(daySessions, b));
  }
  ...
  function getTimeSlots(filteredSessions) {
    ...
    const sessionsByDay = {}; for (const d of days) sessionsByDay[d] = data.filter(s=>s.day===d);
...
function breakRowHtml(label = "DESCANS") {
  ...
  function renderSchedule() {
    ...
    for (const slot of slots) {
      for (const d of days) { const abs = getActiveBreaksForDay(sessionsByDay[d]); for (const b of abs) { if (slot.start === b.start) { html += breakRowHtml(b.label); break; } } }
