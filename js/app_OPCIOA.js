...
// ---------- OPCIÓ A (recomanada): descans com a franja fixa ----------
// El descans forma part de l'horari base (franges), i NO depèn de les sessions filtrades.
// Així surt tant "Per classe" com "Per professor" encara que no hi hagi cap sessió de descans al filtre.
const FIXED_BREAK = { start: "11:20", end: "11:40", label: "DESCANS" };
...
function colorForSubjectItem(s){
  ...
  function getTimeSlots(filteredSessions) {
    ...
    for (const s of filteredSessions) {
      ...
      const subj = String(s.subject ?? "").toUpperCase();
      if (s.type === "break" || subj.includes("DESCANS")) {
        slots.get(key).isBreak = true;
      }
    }
    // ⭐ OPCIÓ A: afegim sempre la franja de descans a les franges horàries
const breakKeyFixed = `${FIXED_BREAK.start}-${FIXED_BREAK.end}`;
if (!slots.has(breakKeyFixed)) {
  slots.set(breakKeyFixed, { start: FIXED_BREAK.start, end: FIXED_BREAK.end, isBreak: true, label: FIXED_BREAK.label });
}
